...
./mvnw spring-boot:run -Dspring-boot.run.profiles=prod   # COS production storage
```

IntelliJ run configurations are pre-made in `.run/` (GameSaves [local], [minio], [prod]).

### Database

MySQL 8.0, database `gamesaving`. JPA `ddl-auto: validate` — entities MUST match table schema. Set credentials via `DB_PASSWORD` env var.

**Flyway 自动迁移** — 所有表结构变更通过版本化 SQL 管理，启动应用自动执行。

```bash
# 首次部署（只需建空库，Flyway 自动建所有表）：
mysql -u root -p -e "CREATE DATABASE IF NOT EXISTS gamesaving CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"

# 日常改表：在 src/main/resources/db/migration/ 下创建 V{version}__{description}.sql，启动应用自动执行。
# 查看迁移历史：
mysql -u root -p -e "SELECT version, description, installed_on, success FROM gamesaving.flyway_schema_history;"
```

迁移文件命名规则：`V{序号}__{描述}.sql`（双下划线分隔），如 `V2__add_user_bio.sql`。现有迁移：
...
| `s3` | `S3StorageServiceImpl` | AWS S3-compatible (MinIO for test, Tencent COS for production). Uses AWS SDK v2. Production (COS) requires env vars: `COS_SECRET_ID`, `COS_SECRET_KEY`, `COS_APPID`. |
...
- Profile configs (`application-{local,minio,prod}.yaml`) wire the right implementation
...
`AdminAuditLog` entity records admin operations (action, target type/id, detail, admin username, IP address). Used for accountability — all admin panel modifications are logged. Table included in Flyway V1.
...
Key-value store for site-wide configuration (`SiteSetting` entity, `site_settings` table). Currently supports `background_image_url` (homepage hero background). `SiteSettingController` provides public GET and admin POST endpoints. Frontend admin panel at `AdminSiteSettings.vue` (`/admin/site-settings`). Table included in Flyway V1.
